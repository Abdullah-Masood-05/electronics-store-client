...
export const AuthProvider = ({ children }) => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                try {
                    ...
                    // If backend fetch fails, still keep Firebase user
                    setFirebaseUser(user);
                    setBackendUser(null);
                }
            } else {
     ...
     * Firebase login → get token → store → fetch backend profile.
     */
     ...
     * Firebase create → get token → create backend user → fetch profile.
     */
    const register = async (email, password, name) => {
        ...
        // Create/update user in backend with name
